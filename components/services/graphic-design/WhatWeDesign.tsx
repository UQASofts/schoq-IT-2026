...
export default function WhatWeDesign() {
  ...
  const t = useTranslations("GraphicDesign.Page.capabilities");
  const items = t.raw("items") as DesignItem[];
  ...
  return (
    <section
      ref={sectionRef}
      className="flex w-full flex-col items-start justify-center overflow-hidden bg-white px-[4%] py-global font-sans sm:py-global-sm md:py-global-md lg:px-[8%] lg:py-global-lg xl:py-global-xl 2xl:py-global-2xl"
    >
      <div className="mx-auto w-full max-w-7xl space-y-10 md:space-y-14">
        ...
        <div ref={listRef} className="flex w-full flex-col border-t border-[#E6E9EE]">
          {items.map((item) => (
            <div
              key={item.number}
            ...
            >
              ...
              <ArrowRight className="hidden h-4 w-4 shrink-0 text-neutral-400 md:block" />
            </div>
          ))}
        </div>
      </div>
